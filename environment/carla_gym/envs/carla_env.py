...
class CarlaEnv(gym.Env):
    def __init__(self, config=ENV_CONFIG, z_size = 512):
        """
        ...
        if self.config["use_image_only_observations"]:
            ...
            self.observation_space = Tuple(
                [image_space,
                 ...
                 Box(np.finfo(np.float32).min, np.finfo(np.float32).max, shape=(1,self.z_size), dtype=np.float32)])  
    ...
    def step_env(self, action):
        ...
        throttle = float(np.clip(action[1], 0.1, 1))
